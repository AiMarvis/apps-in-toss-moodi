...
export const generateMusic = functions
  ...
  .https.onCall(async (data: GenerateMusicRequest, context) => {
    ...
    // 감정 키워드 유효성 검사
    const validEmotions: EmotionKeyword[] = [
    ...
    try {
          ...
          if (trackDoc.exists) {
            ...
            if (track.pairId) {
              ...
              if (!pairSnapshot.empty) {
                pairTrack = pairSnapshot.docs[0].data() as Track;
              }
            }
            return {
              status: 'complete',
      ...
      if (taskStatus === 'SUCCESS' && taskData?.response?.data?.[0]?.audio_url) {
        ...
        for (let i = 0; i < responseDataArray.length; i++) {
          ...
          if (!audioInfo?.audio_url) continue;
...
export const sunoCallback = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      if (callbackType === 'complete' && responseData?.length > 0 && responseData[0]?.audio_url) {
 ...
 * - 트랜잭션으로 크레딧 지급 + 주문 기록
 */
export const grantCredits = functions.https.onCall(
  async (data: GrantCreditsRequest, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', '로그인이 필요해요');
    }
    ...
    if (orderDoc.exists && orderDoc.data()?.granted) {
